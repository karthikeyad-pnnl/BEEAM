...
partial model AC_LoadBaseWye
  ...
  parameter Modelica.Units.SI.ReactivePower Q_nom = 0 "Nominal reactive power" annotation(
    Dialog(group = "Device Ratings"));
  parameter Modelica.Units.SI.Voltage V_nom = 277 "Nominal AC operating voltage (line-neutral)" annotation(
    Dialog(group = "Device Ratings"));
  final parameter Modelica.Units.SI.ComplexPower S_nom = Complex(P_nom, Q_nom) "Nominal AC complex power";
  final parameter Modelica.Units.SI.Current I_nom = Modelica.ComplexMath.abs(S_nom) / 3 / V_nom "Nominal AC current";
  // Initialization
  parameter Modelica.Units.SI.Angle vAngle_init = 0 "Nominal phase A voltage angle for solver init" annotation(
    Dialog(group = "Initialization"));
  final parameter Modelica.Units.SI.ComplexVoltage VA_init = Modelica.ComplexMath.fromPolar(V_nom, vAngle_init);
  final parameter Modelica.Units.SI.ComplexVoltage VB_init = Modelica.ComplexMath.fromPolar(V_nom, vAngle_init - Modelica.Units.Conversions.from_deg(120));
  final parameter Modelica.Units.SI.ComplexVoltage VC_init = Modelica.ComplexMath.fromPolar(V_nom, vAngle_init + Modelica.Units.Conversions.from_deg(120));
  final parameter Modelica.Units.SI.ComplexCurrent IA_init = Modelica.ComplexMath.conj(S_nom / 3 / VA_init);
  final parameter Modelica.Units.SI.ComplexCurrent IB_init = Modelica.ComplexMath.conj(S_nom / 3 / VB_init);
  final parameter Modelica.Units.SI.ComplexCurrent IC_init = Modelica.ComplexMath.conj(S_nom / 3 / VC_init);
