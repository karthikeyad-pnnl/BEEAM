...
model ACDC_3pInverterSimpleGridForming
  ...
  final parameter Modelica.Units.SI.Angle vAngle_ref_B = vArg_ref - Modelica.Units.Conversions.from_deg(120);
  final parameter Modelica.Units.SI.Angle vAngle_ref_C = vArg_ref + Modelica.Units.Conversions.from_deg(120);
  ...
  parameter Real gamma = 0.0 "Loss model quadratic term (per-unit)" annotation(
    Dialog(group = "Converter Loss Model"));
  parameter Modelica.Units.SI.Power P_stby = 0 "Standby loss" annotation(
    Dialog(group = "Converter Loss Model"));
  parameter Modelica.Units.SI.Power P_ACmin = 1 "Minimum output power (2-stage loss model)" annotation(
    Dialog(group = "Converter Loss Model"));
