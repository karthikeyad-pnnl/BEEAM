...
model ACDC_1pBidirectionalSimple
  ...
  parameter Real alpha_ACDC = 0.0 "AC/DC loss model constant term (per-unit)" annotation(
    Dialog(group = "Converter Loss Model"));
  parameter Real beta_ACDC = 0.1 "AC/DC loss model linear term (per-unit)" annotation(
    Dialog(group = "Converter Loss Model"));
  parameter Real gamma_ACDC = 0.0 "AC/DC loss model quadratic term (per-unit)" annotation(
    Dialog(group = "Converter Loss Model"));
  parameter Real alpha_DCAC = 0.0 "DC/AC loss model constant term (per-unit)" annotation(
    Dialog(group = "Converter Loss Model"));
  parameter Real beta_DCAC = 0.1 "DC/AC loss model linear term (per-unit)" annotation(
    Dialog(group = "Converter Loss Model"));
  parameter Real gamma_DCAC = 0.0 "DC/AC loss model quadratic term (per-unit)" annotation(
    Dialog(group = "Converter Loss Model"));
...
// Loss calculation (with smooth transition across zero)
  P_Loss = HPF.PowerConverters.HelperFunctions.homotopyTransition(P_AC, 0, alpha_ACDC * P_nom, P_nom * (alpha_DCAC + beta_DCAC * (-P_AC / P_nom) + gamma_DCAC * (-P_AC / P_nom) ^ 2), P_nom * (alpha_ACDC + beta_ACDC * (P_DC / P_nom) + gamma_ACDC * (P_DC / P_nom) ^ 2));
