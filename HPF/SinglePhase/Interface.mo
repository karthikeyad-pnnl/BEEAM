...
/*
  ...
  partial model ACDC_ConverterBase
    outer SystemDef systemDef;
    // Ratings
    parameter Modelica.Units.SI.Voltage V_Rect(start = 0) = 1 "Rectifier DC output" annotation(
      Dialog(group = "Device Ratings"));
    parameter Modelica.Units.SI.Power nomP = 50 "Rated nominal power" annotation(
      Dialog(group = "Device Ratings"));
    parameter Modelica.Units.SI.Voltage nomV = 120 "Nominal AC operating voltage" annotation(
      Dialog(group = "Device Ratings"));
    final parameter Modelica.Units.SI.Current nomI = nomP / nomV "Nominal AC operating current";
    // Initialization
    parameter Modelica.Units.SI.Angle vAngle = 0 "Nominal voltage angle for solver init" annotation(
      Dialog(group = "Initialization"));
    // Loss Model
    parameter Modelica.Units.SI.Power P_stby = 0 "Standby (no load) input AC power" annotation(
      Dialog(group = "Converter Loss Model"));
    parameter Modelica.Units.SI.Power P_DCmin = 0.5 "Minimum converter power (2-stage loss model)" annotation(
      Dialog(group = "Converter Loss Model"));
