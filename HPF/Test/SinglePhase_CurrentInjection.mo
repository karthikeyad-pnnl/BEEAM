...
model SinglePhase_CurrentInjection
  inner SystemDef systemDef(hrms = {1, 3}) annotation(
    Placement(transformation(extent = {{-56, 32}, {-36, 52}})));
  ...
  SinglePhase.Components.Impedance z "5 + 2*j" annotation(
    Placement(transformation(extent = {{-20, 10}, {0, 30}})));
  SinglePhase.Components.Impedance z1(z = 2 + 1 * j) annotation(
    Placement(transformation(extent = {{-10, -10}, {10, 10}}, rotation = 270, origin = {10, -14})));
  SinglePhase.Components.Ground ground annotation(
    Placement(transformation(extent = {{-50, -44}, {-30, -24}})));
  HPF.SinglePhase.Sources.CurrentSource currentSource1(iReal = {5, 2}, iImag = {0.25, 0.5}) annotation(
    Placement(visible = true, transformation(origin = {38, -8}, extent = {{-10, -10}, {10, 10}}, rotation = 270)));
equation
  ...
  connect(v.pin_p, z.pin_p) annotation(
    Line(points = {{-40, 0}, {-40, 20}, {-20, 20}}, color = {0, 0, 0}));
  connect(v.pin_n, ground.pin) annotation(
    Line(points = {{-40, -20}, {-40, -22.75}}, color = {117, 80, 123}));
