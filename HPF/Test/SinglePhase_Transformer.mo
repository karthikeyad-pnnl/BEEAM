...
model SinglePhase_Transformer
  ...
  Modelica.Electrical.QuasiStatic.SinglePhase.Ideal.IdealTransformer T_Quasi(n = 480 / 208) annotation(
    Placement(visible = true, transformation(origin = {8, -26}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Electrical.QuasiStatic.SinglePhase.Sources.VoltageSource vSrc_Quasi(V = 480, f = 60, phi = 0) annotation(
  ...
  Modelica.Electrical.QuasiStatic.SinglePhase.Basic.Impedance impedance(Z_ref = 1 + 0 * j) annotation(
    Placement(visible = true, transformation(origin = {-22, -16}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Electrical.QuasiStatic.SinglePhase.Basic.Impedance impedance1(Z_ref = 10 + 5 * j) annotation(
...
equation
  ...
  connect(vSrc_HPF.pin_n, T_HPF.pinN_Prim) annotation(
    Line(points = {{-48, 30}, {-26, 30}, {-26, 30}, {-2, 30}}, color = {117, 80, 123}));
  annotation(
