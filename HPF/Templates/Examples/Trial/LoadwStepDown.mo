...
model LoadwStepDown
    extends HPF.Templates.LoadwStepDown(
      redeclare HPF.DC.DC2DC_Converters.StepDown dcdc_Converter(
        ...
        name=modelData.stepDownName),schedule(tableOnFile = fill(true,nLoad),table = fill(0.0,15,2),fileName = ModelicaServices.ExternalReferences.loadResource(modelData.fileName)));
